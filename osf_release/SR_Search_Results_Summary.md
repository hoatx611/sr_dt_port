...
**File:** SR_Search_Results_Summary.md
...
**Anchor:** `SR_Search_Strategy_FINAL.md` v1.0 §9 (trạng thái tìm kiếm)
...
- *Scopus raw = 3.422 theo `dedup_report.txt`; per-file sum §2.1 = 3.421 (chênh 1 record do T8_Scopus.ris có ER tag không chuẩn từ Publish or Perish export).*
- *Springer raw = 904 theo `dedup_report.txt`; `ris_integrity_report.csv` ghi 905 (chênh 1 record do parser edge case).*
- *Số liệu `dedup_report.txt` là authoritative. Tổng 7.847 và unique 5.822 không bị ảnh hưởng.*
...
| File | Mô tả | Bước |
|------|-------|------|
| `search_results/*.ris` | 23 RIS files gốc | 4–6 |
...
| File | Quan hệ |
|------|---------|
| `SR_Search_Strategy_FINAL.md` §9 | Status table (đã cập nhật với số liệu thực) |
| `SR_Deviation_Log.md` | DEV-002 (filename mapping), DEV-004 (Q10 — Resolved) |
| `WorkPlan_FINAL.md` | Bước 4–7 trạng thái |
...
| v1.1 | 2026-05-15 | Q10 Vintage chạy xong. Cập nhật: 23 files, 7.847 raw, 5.822 unique, 2.025 dedup, Pha A 284/5.061/477, Rayyan 5.345. DEV-004 Resolved. |
| **v1.2** | **2026-05-19** | **Kiểm toán: Missing abstract 1.417→1.422. Mở rộng footnote §3 ghi rõ Springer discrepancy. Không ảnh hưởng tổng số liệu.** |

---

*SR_Search_Results_Summary.md — v1.2 — 2026-05-19 — NCS HoaTX*
