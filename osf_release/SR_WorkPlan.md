...
- **Nhãn nội bộ ""** chỉ dùng trong workspace cá nhân — KHÔNG xuất hiện trong: (i) bản thảo manuscript, (ii) tài liệu OSF, (iii) supplementary nộp tạp chí.
- Khi viết text cho OSF / manuscript / supplementary → strip "" và dùng `osf_release/` làm thư mục bản công khai.
...
- File trung tâm: internal knowledge graph (workspace-only, not part of this public release).
- Mọi tri thức / kết quả / quyết định thiết kế PHẢI cập nhật vào internal knowledge graph sau mỗi bước.
...
|------------|-----------|-------|
| workspace internal folder (trừ `osf_release/`, `bak/`) | Hàng tuần Chủ nhật 22:00 | Google Drive / GitHub private |
| RIS files trong `search_results/` | 1 lần sau Bước 7 | OSF Files |
| Rayyan project state | Weekly snapshot CSV | `03_screening/Rayyan_weekly_snapshots/` |
| OSF Project state | Sau mỗi mốc Bước | `SR_OSF_Registration.md` |
...
> **Bộ 5 RQ chính thức** lưu trong file riêng `SR_Research_Questions.md` v1.0. WorkPlan KHÔNG duplicate RQ — chỉ tham chiếu. Mọi sửa đổi RQ phải bắt đầu từ file SoT đó, sau đó propagate xuống WorkPlan + Strategy + Extraction Form.

### Tóm tắt RQ (chi tiết xem RQ doc)

| RQ | Tên | Sub | Bước giải quyết | Output |
|----|-----|------|------------------|---------|
| **RQ1** | Bức tranh tổng thể (WHAT) | 1a–1e | 16 (Bibliometric) | `SR_Bibliometric.md` + 5 PNG |
...
| **RQ5** | Chương trình NC + Lộ trình (WHAT NEXT) — conditional roadmap | 5a–5d | 17.5, 17.6 | `SR_Research_Agenda.md` |
...
- Items không áp dụng (12, 13e, 14, 15, 19, 20c, 21, 22) phải có **justification tường minh** trong `SR_PRISMA_Checklist.md` và mục Limitations.
...
- **`SR_Deviation_Log.md`** — log mọi sai lệch so với WorkPlan/Protocol (8 trường). Source duy nhất cho Bước 20.3 OSF Amendments.
- **`SR_Risk_Register.md`** — 15 rủi ro với likelihood × impact × mitigation × Recovery trigger.
...
| Pha | Bước | Trạng thái |
|-----|-------|-------------|
| Pha 1 — Thiết kế | 1–3 | 🔄 Đang triển khai (Bước 1.1, 1.3, 1.6 ✅; còn 1.2, 1.4, 1.5, 1.7–1.13) |
...
**Trạng thái tổng:** 0/22 bước hoàn tất; Bước 1 đang triển khai (3/13 sub-task ✅).
...
| 1.3 Soạn Registration submission text (PRISMA-P style 20 mục) | Claude | ✅ 2026-05-10 — [SR_OSF_Registration_Submission.md](#checklist/sr_dt_port/SR_OSF_Registration_Submission.md) v1.0 |
| 1.4 NCS review + chỉnh sửa Registration text | NCS | ⬜ |
| 1.5 Submit Registration → nhận DOI 10.17605/OSF.IO/* | NCS | ⬜ |
| 1.6 Cập nhật OSF Project ID vào header 5 file v1.0 (RQ + Strategy + WorkPlan + Eligibility + Extraction Form) | Claude | ✅ 2026-05-10 |
| 1.7 Tạo `osf_release/` với 3 file đã strip nhãn nội bộ | Claude | ⬜ |
| 1.8 Cập nhật OSF Registration DOI vào header 5 file v1.0 | Claude | ⬜ |
| 1.9 NCS upload `osf_release/` lên OSF Files | NCS | ⬜ |
...
| 1.13 Cập nhật Rayyan project ID vào `SR_OSF_Registration.md` | NCS | ⬜ |
...
**Exit criteria:** `SR_Eligibility_Criteria.md` v1.0 phê duyệt; PICOC khung; 28 mã EC/IC đầy đủ; 6 borderline rules.
...
**Output cuối:** `SR_Eligibility_Criteria.md` v1.0 phê duyệt.
...
**Exit criteria:** 20 lượt query (10 Scopus + 10 IEEE) chạy đủ; RIS files lưu trong `search_results/`; số records mỗi query ghi vào Strategy §9.
...
| Việc | Mô tả | Trạng thái |
|------|--------|-------------|
| 7.1 Liệt kê toàn bộ RIS files trong `search_results/` | Claude | ⬜ |
...
| 8.1.3 Chạy `screening.py` trên `dedup_unique.ris` → output `screening_pre.csv` + `.ris` (PRE_LABEL trong N1) + `screening_distribution.txt` | NCS + Claude | ⬜ |
...
| Việc | Mô tả | Trạng thái |
|------|--------|-------------|
| 8.2.1 NCS upload `screening_pre.ris` vào Rayyan project | NCS | ⬜ |
...
**Output Bước 8:** `ta_screening.csv` (n × 4 cột: doi, title, ta_decision, ta_ec_code).
...
Rà mục lục 2024–2026 → ghi vào `hand_search.csv`.

**Output Bước 9:** `included_studies.csv` (kỳ vọng 80–150 studies).
...
| 10.4 Lưu `prisma_flow.png` + source | NCS | ⬜ |
...
#### 11.1 Form structure (xem `SR_Data_Extraction_Form.md` v1.0 — 6 nhóm × 40 trường)
...
NCS chấm tuần tự included pool, ưu tiên cảng VN/ĐNÁ + cảng nguồn lực hạn chế. Output: `extraction.csv`.
...
#### 12.1 Maturity rubric (`SR_Classification_Rubric.md`)
...
NCS chấm từng study; output `quality_appraisal.csv` + `borderline_decisions_log.md`.
...
| Việc | Mô tả | Trạng thái |
|------|--------|-------------|
| 15.1.1 Lập 78 seed list (lưu trong internal knowledge graph) | Claude | ⬜ |
...
**Pre-conditions:** Bước 11 done.
**Exit criteria:** `SR_Bibliometric.md` + 5 hình VOSviewer (PNG).
...
6 sub-steps: Familiarisation → Initial coding → Theme search → Theme review → Define themes → Output `thematic_synthesis.md`.
...
| `prisma_flow.png` + source | Flow diagram |
| `dedup.py` + `dedup_unique.ris` | Reproducibility code + data |
| `screening.py` + `tf_journal_exclusion.py` | Pha A pre-screen |
| `included_studies.csv` | Final pool |
| `extraction.csv` | Trích xuất |
| `quality_appraisal.csv` | Q1–Q8 scores |
...
Tổng hợp từ `SR_Deviation_Log.md` → submit từng Amendment trên OSF Project.
...
Output: `expert_validation.md` + 3–5 expert confirmation letters.
...
- **Documentation:** DEV-NNN trong `SR_Deviation_Log.md` + OSF Amendment
...
## VI.1 Risk Register (chi tiết trong `SR_Risk_Register.md`)
...
3. internal knowledge graph đã có node mới
4. SR_Deviation_Log.md cập nhật nếu có sai lệch
...
| Search strategy reproducible | ⬜ | `SR_Search_Strategy.md` §4 (12 + 10 query nguyên văn) |
| Dedup pipeline open | ⬜ | `dedup.py` + `requirements.txt` |
| T/A pre-screen open | ⬜ | `screening.py` + `tf_journal_exclusion.py` |
| Eligibility operationalized | ⬜ | `SR_Eligibility_Criteria.md` (28 EC/IC + 6 borderline) |
| Quality appraisal protocol | ⬜ | `SR_Quality_Appraisal.md` |
| Data extraction form | ⬜ | `SR_Data_Extraction_Form.md` v1.0 |
| Maturity rubric | ⬜ | `SR_Classification_Rubric.md` |
| Borderline log | ⬜ | `borderline_decisions_log.md` |
| Final pool data | ⬜ | `included_studies.csv` |
| Extraction data | ⬜ | `extraction.csv` |
| Bibliometric maps | ⬜ | `vos_*.png` + VOSviewer config |
| Synthesis output | ⬜ | `SR_Bibliometric.md` + `SR_Research_Agenda.md` |
| PRISMA Checklist 27/27 | ⬜ | `SR_PRISMA_Checklist.md` |
...
| File | Vai trò |
|------|---------|
| `SR_Research_Questions.md` v1.0 | Single Source of Truth cho RQ — WorkPlan reference §0.7 |
| `SR_Search_Strategy.md` v1.0 | Bước 4–7, 15 — chiến lược tìm kiếm + PRESS |
| `SR_Eligibility_Criteria.md` | Bước 2, 8, 9 — 28 EC/IC + 6 borderline rules |
| `SR_Screening_Protocol.md` | Bước 8 — Pha A Python + Pha B Rayyan |
| `SR_Data_Extraction_Form.md` v1.0 | Bước 11 — 6 nhóm × 40 trường |
| `SR_Quality_Appraisal.md` | Bước 13 — Kitchenham 8 criteria |
| `SR_Classification_Rubric.md` | Bước 12 — Maturity rubric L1-L5 |
| `SR_PRISMA_Checklist.md` | Bước 19 — 27/27 + 8 N/A |
| `SR_OSF_Registration.md` | Bước 1, 20 — OSF tracker + Amendments |
| `SR_Deviation_Log.md` | Bước 0.9, 20 — DEV log |
| `SR_Risk_Register.md` | Bước 0.9 — 15 rủi ro |
| `osf_release/` | Bước 20 — bản công khai đã strip nhãn nội bộ |
| internal knowledge graph | Đồ thị tri thức trung tâm |

---

*WorkPlan.md — v1.0 — 2026-05-08 — anchored on RQ_v1.0 (`SR_Research_Questions.md`) + PRISMA 2020 + PRISMA-S + Kitchenham 2007 + PRESS + Q1 target IJPR/MPM*
